...
interface SubtitleTrackProps {
	subtitles: Subtitle[];
	duration: number;
	pausedTransformX?: number; // only needed when paused
	playing: boolean;
	pxPerSecond: number;
	animationKey: number;
}
...
export const SubtitleTrack = React.memo(function SubtitleTrack({
	...
	const style = playing
		? {
				...
				animationDelay: `-${animationKey / 4}s`, // match frame time
				animationPlayState: "running",
		  }
		: {
				transform: `translateX(-${pausedTransformX}px)`,
		  };
		...
		<>
			...
			>
				{subtitles.map((sub, i) => {
					...
					return (
						<div key={i} className="absolute h-full pr-4" style={{ width: `${width}px`, left: `${left}px` }}>
...
export function SubtitleScroller({ subtitles, duration, currentTime, playing }: SubtitleScrollerProps) {
	...
	const pixelOffset = currentTime * pxPerSecond;
	...
	return (
		<div className="relative overflow-hidden h-4 mt-4">
			...
			<SubtitleTrack
				...
				duration={duration}
				pausedTransformX={playing ? undefined : pixelOffset}
				playing={playing}
